# @title Combine Data Loading, Environment Definition, and Instantiation
...
from torchrl.data.tensor_specs import (
    ...
    DiscreteTensorSpec as DataDiscreteTensorSpec, # Rename to avoid conflict
    MultiDiscreteTensorSpec as DataMultiDiscreteTensorSpec, # Rename to avoid conflict
    BoxList as DataBoxList, # Rename to avoid conflict
    CategoricalBox as DataCategoricalBox, # Rename to avoid conflict
    UnboundedContinuous as DataUnboundedContinuous # Rename to avoid conflict
)
...
# Ensure the environment directory is in the path for imports
import sys
if '/content/anfuelpriceenv' not in sys.path:
    sys.path.append('/content/anfuelpriceenv')
...
class FuelpriceenvfeatureGraph():
    ...
    def _load_data(self):
        ...
        local_data_path = '/content/drive/MyDrive/deep learning codes/EIAAPI_DOWNLOAD/solutions/mergedata/Cleaneddata.csv'
...
class SimpleMultiAgentPolicyModuleGCN(nn.Module):
     ...
     def forward(self, x, edge_index, prev_rnn_hidden_state=None):
         ...
         batch_size_flat = num_envs * num_agents
         ...
         offsets = torch.arange(num_envs, device=x.device).view(-1, 1, 1) * num_agents # Shape [num_envs, 1, 1]
         edge_index_offset = edge_index_permuted + offsets # Shape [num_envs, num_edges_per_graph, 2]
         ...
         if prev_rnn_hidden_state is None:
              prev_rnn_hidden_state = self._initial_rnn_hidden_state.expand(1, batch_size_flat, -1).to(x.device)

         rnn_output, next_rnn_hidden_state = self.rnn(rnn_input, prev_rnn_hidden_state.contiguous())
...
class SimpleMultiAgentValueModuleGCN(nn.Module):
    ...
    def forward(self, x, edge_index, prev_rnn_hidden_state=None):
        ...
        gcn_output_flat = self.gcn1(x_flat, edge_index_flat)
        rnn_input = gcn_output_flat.reshape(1, batch_size_flat, self.gcn_hidden_dim)
...
# Redefine the custom combined module as nn.Module with corrected initial state handling
class CombinedPolicyForecastingBase(nn.Module):
    ...
    def forward(self, x, edge_index, prev_policy_rnn_hidden_state=None, prev_forecast_rnn_hidden_state=None):
        num_envs = x.shape[0]
        batch_size_flat = num_envs * self.num_agents

        policy_rnn_input_state = prev_policy_rnn_hidden_state
        if policy_rnn_input_state is not None:
             policy_rnn_input_state = policy_rnn_input_state.reshape(1, batch_size_flat, self.hidden_rnn_dim);

        forecast_rnn_input_state = prev_forecast_rnn_hidden_state
        if forecast_rnn_input_state is not None:
             forecast_rnn_input_state = forecast_rnn_input_state.reshape(1, batch_size_flat, self.hidden_rnn_dim);

        action_logits, next_policy_rnn_hidden_state_rnn_format = self.policy_module_base(x, edge_index, policy_rnn_input_state);
        forecast, next_forecast_rnn_hidden_state_rnn_format = self.forecasting_module_base(x, edge_index, forecast_rnn_input_state);

        next_policy_rnn_hidden_state = next_policy_rnn_hidden_state_rnn_format.reshape(num_envs, self.num_agents, self.hidden_rnn_dim);
        next_forecast_rnn_hidden_state = next_forecast_rnn_hidden_state_rnn_format.reshape(num_envs, self.num_agents, self.hidden_rnn_dim);

        return action_logits, next_policy_rnn_hidden_state, forecast, next_forecast_rnn_hidden_state


# Instantiate the base policy and forecasting nn.Modules
if env is not None:
    ...
    print("\nInstantiating Corrected Custom Combined Base nn.Module (handling initial state)...")
    try:
        ...
        combined_module_base_gcn = CombinedPolicyForecastingBase(
            policy_module_base=policy_module_gcn,
            forecasting_module_base=forecasting_module_gcn,
            hidden_rnn_dim=hidden_rnn_dim,
            num_agents=num_agents,
            device=device
        ).to(device)

        print("Corrected Custom Combined Base nn.Module instantiated successfully.")
        ...
        print("Custom Combined TensorDictModule wrapped successfully.")
        ...
        print("Value TensorDictModule wrapped successfully.")
        ...
        print(f"\nAn error occurred during TensorDictModule wrapping or instantiation: {e}")
    ...
    combined_module_wrapped_gcn = None
    value_net_gcn = None
    print("\nEnvironment is None. Cannot verify TensorDictModules or instantiate collector.")
...
if combined_module_base_gcn is not None and value_net_gcn is not None:
    ...
    def register_module_hooks(module, name_prefix):
        print(f"Attempting to register hooks for module {name_prefix}...")
        if hasattr(module, 'named_parameters'):
            ...
            if len(module_params) > 0:
                ...
                print(f"No parameters found in module {name_prefix}.")
        else:
            print(f"Module {name_prefix} does not have named_parameters().")

    if hasattr(combined_module_base_gcn, 'policy_module_base'):
        register_module_hooks(combined_module_base_gcn.policy_module_base, "policy_module_base")
    if hasattr(combined_module_base_gcn, 'forecasting_module_base'):
        register_module_hooks(combined_module_base_gcn.forecasting_module_base, "forecasting_module_base")
    if hasattr(value_net_gcn, 'module'):
...
# Now, attempt the test forward and backward pass again
if combined_module_wrapped_gcn is not None and value_net_gcn is not None and env is not None:
    ...
    try:
        ...
        collected_tensordict = temp_collector.rollout(1)
        ...
        states_at_t0 = collected_tensordict[0]
        ...
        value_output_td = value_net_gcn(states_at_t0.clone())
        ...
        print("\nRe-running Combined Policy/Forecasting network forward pass on initial state...")
        required_combined_keys = combined_module_wrapped_gcn.in_keys
        if not all(initial_tensordict.has(key) for key in required_combined_keys):
             print(f"Error: Not all required keys for combined net found in initial_tensordict. Missing: {[key for key in required_combined_keys if not initial_tensordict.has(key)]}")
             print("Skipping combined module loss calculation due to missing keys in initial_tensordict.")
             dummy_policy_loss = torch.tensor(0.0, device=device)
             dummy_forecasting_loss = torch.tensor(0.0, device=device)
             print("Using zero dummy losses for combined module.")
        else:
            combined_input_td = initial_tensordict.select(*required_combined_keys).clone()
            combined_output_td = combined_module_wrapped_gcn(combined_input_td)

            dummy_policy_loss = combined_output_td.get(('agents', 'action')).sum()
            dummy_forecasting_loss = combined_output_td.get('forecast').sum()

            print(f"Dummy policy loss: {dummy_policy_loss.item()}")
...
# The hook_handles list is available for removing hooks later.
# for handle in hook_handles:
#     handle.remove()
# print("\nGradient hooks removed.")
